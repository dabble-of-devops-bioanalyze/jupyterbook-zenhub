...
def main(source_folder_path, archive_book_flag):
    ...
    zendesk_category_id = check_category_on_zendesk(
        hc, zendesk_category_name
    )  # will exit program if not found
    ...
    html_files_for_zendesk = handle_sections_on_zendesk(
    ...
    for f in html_files_for_zendesk:
        ...
        article_dict = update_urls_in_article_dict(
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename=log_file_path, level=logging.INFO, filemode="w", format=LOGGING_FORMAT
    )
